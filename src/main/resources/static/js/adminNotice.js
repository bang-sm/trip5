...
$(document).on(
		...
		function() {
			...
			$(this).parents(".sNoticecontent").children('.nSlideCheck').prop(
					"checked", false);
			$.ajax({
...
$(document).on(
		...
		function() {
			...
			$("input:checkbox[class='nSlideCheck']:checked").parents(
					".sNoticecontent").children('.sNoticeUid').each(
					function() {
						snId.push($(this).text());
						if ($(this).parents(".sNoticecontent").children(
								".nSlideInput").val().trim() == "") {
							cnt++;
						} // end if
					})
...
$(document).on(
		...
		function() {
			...
			$.ajax({
				...
				type : "POST",
				success : function(data) {
					console.log(data[0].pnHeader + " dkfjaksdjfkadsjfk");
					$('.title').val(data[0].pnHeader);
					$('.textPopUpNotice').val(data[0].pnContent);
					$('.pnTop').val(data[0].pnTop);
					$('.pnLeft').val(data[0].pnLeft);
					$('.pnWidth').val(data[0].pnWidth);
					$('.pnHeight').val(data[0].pnHeight);
					$('.pnDate').val(data[0].pnDate);
				}
			}) // Ajax 끝
...
$(document).on(
		...
		function() {
			...
			$("input:checkbox[class='nPopUpCheck']:checked").parents(
					".nPopUpcontent").children('.pNoticeUid').each(
					function() {
						pnId.push($(this).text());
						if ($(this).parents(".nPopUpcontent").children(
								".nPopUpInput").val().trim() == "") {
							cnt++;
						}
					})
